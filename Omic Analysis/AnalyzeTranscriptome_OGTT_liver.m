%% Load files
load('./Resource/TranscriptomeOGTT_liver.mat');
Transcriptome_liver(~logical(Transcriptome_liver)) = NaN;
...
col_num_liver = [11,5,5,5,5,12,5,5,5,5];
threshold_q = 0.1;
...
for i = 1:size(is_EnoughData,2)
    logic_EnoughData = CheckEnoughData(Transcriptome_liver(:,i), col_num_liver);
    is_EnoughData(index_liver,i) = logic_EnoughData;
end

%%% Calculation
%%% Mean
mean_liver = NaN(length(col_num_liver),size(Info_TranscriptomeName,1));
% liver
for i = 1:size(mean_liver,2)
    mean_liver(:,i) = GetMean(Transcriptome_liver(:,i),col_num_liver);
end

%%% FC & log2FC (WT tX/t0, ob tX/t0, ob/WT tX)(x = 0,2,4,6,8,12,24)
FC_liver = 2.^log2FC_liver;
...
% {"<8 h","8-16 h",">16 h"}
THalf_liver = NaN(num_Genotype,size(Info_TranscriptomeName,1));
...
%cutoff = 0.5;
temp_num_Cluster = 3;
name_figure_1 = 'Transcriptome_liver';
name_figure_2 = 'Transcriptome_MeanTimeCourseInOne_liver';
%name_figure_suffix = ['_',num2str(cutoff),'MaxLinkage'];
name_figure_suffix = ['_',num2str(temp_num_Cluster),'_Clusters'];

%% Transcriptome_liver_WT&OB
%%% Dendrogram
