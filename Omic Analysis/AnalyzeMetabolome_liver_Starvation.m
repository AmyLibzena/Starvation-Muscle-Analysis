%% Load files
load('./Resource/MetabolomeData_liver.mat');
index_F16P = find(strcmp(Info_MetabolomeName(:,2),'F1,6P'));
index_F26P = size(Info_MetabolomeName,1) + 1;
Metabolome_liver(:,index_F16P) = Extra_ICQEMS(:,1);
Metabolome_liver(:,index_F26P) = Extra_ICQEMS(:,2);
Info_MetabolomeName{index_F26P,1} = 'C00665';
Info_MetabolomeName{index_F26P,2} = 'F2,6P';
...
for i = 1:size(is_EnoughData,2)
    logic_EnoughData = CheckEnoughData(Metabolome_liver(:,i), col_num_liver);
    is_EnoughData(index_liver,i) = logic_EnoughData;
end

%% Calculation
%%% Mean
mean_liver = NaN(length(col_num_liver),size(Info_MetabolomeName,1));
for i = 1:size(mean_liver,2)
    mean_liver(:,i) = GetMean(Metabolome_liver(:,i),col_num_liver);
end
...
p_liver = NaN(num_Genotype,size(Info_MetabolomeName,1));
temp_num = sum(col_num_liver(1:num_TimePoint));
% Liver - WT
for i = 1:size(Info_MetabolomeName,1)
    p_liver(index_WT,i) = anova1(Metabolome_liver(1:temp_num,i),Group(1:temp_num),'off');
end
% Liver - OB
for i = 1:size(Info_MetabolomeName,1)
    p_liver(index_OB,i) = anova1(Metabolome_liver(temp_num+1:end,i),Group(temp_num+1:end),'off');
end
% Liver - OB/WT
for i = 1:size(Info_MetabolomeName,1)
    for j = 1:num_TimePoint
        if j == 1
            temp_list_WT = Metabolome_liver(1:col_num_liver(j),i);
            temp_list_OB = Metabolome_liver(temp_num+1:temp_num+col_num_liver(num_TimePoint+j),i);
        else
            temp_list_WT = Metabolome_liver(sum(col_num_liver(1:j-1))+1:sum(col_num_liver(1:j)),i);
            temp_list_OB = Metabolome_liver(sum(col_num_liver(1:num_TimePoint+j-1))+1:sum(col_num_liver(1:num_TimePoint+j)),i);
        end
...
% {"<8 h","8-16 h",">16 h"}
THalf_liver = NaN(num_Genotype,size(Info_MetabolomeName,1));
...
index_AMP = 182;
% WT
ATP = Metabolome_liver(1:num_Replicate*num_TimePoint,index_ATP);
ADP = Metabolome_liver(1:num_Replicate*num_TimePoint,index_ADP);
AMP = Metabolome_liver(1:num_Replicate*num_TimePoint,index_AMP);
...
% OB
ATP = Metabolome_liver(num_Replicate*num_TimePoint+1:end,index_ATP);
ADP = Metabolome_liver(num_Replicate*num_TimePoint+1:end,index_ADP);
AMP = Metabolome_liver(num_Replicate*num_TimePoint+1:end,index_AMP);
...
for j = 1:num_TimePoint
    if j == 1
        temp_list_WT = EC_WT(1:col_num_liver(j));
        temp_list_OB = EC_OB(1:col_num_liver(j));
    else
        temp_list_WT = EC_WT(sum(col_num_liver(1:j-1))+1:sum(col_num_liver(1:j)));
        temp_list_OB = EC_OB(sum(col_num_liver(1:j-1))+1:sum(col_num_liver(1:j)));
    end
...
for j = 1:num_TimePoint
    if j == 1
        temp_list_WT = AMP2ATP_WT(1:col_num_liver(j));
        temp_list_OB = AMP2ATP_OB(1:col_num_liver(j));
    else
        temp_list_WT = AMP2ATP_WT(sum(col_num_liver(1:j-1))+1:sum(col_num_liver(1:j)));
        temp_list_OB = AMP2ATP_OB(sum(col_num_liver(1:j-1))+1:sum(col_num_liver(1:j)));
    end
...
for j = 1:num_TimePoint
    if j == 1
        temp_list_WT = ADP2ATP_WT(1:col_num_liver(j));
        temp_list_OB = ADP2ATP_OB(1:col_num_liver(j));
    else
        temp_list_WT = ADP2ATP_WT(sum(col_num_liver(1:j-1))+1:sum(col_num_liver(1:j)));
        temp_list_OB = ADP2ATP_OB(sum(col_num_liver(1:j-1))+1:sum(col_num_liver(1:j)));
    end
...
%%% Draw
bar_data_muscle_WT = [sum(THalf_liver(index_WT,:)<threshold_THalf_1),sum((THalf_liver(index_WT,:)>=threshold_THalf_1).*(THalf_liver(index_WT,:)<=threshold_THalf_2)),sum(THalf_liver(index_WT,:)>threshold_THalf_2)];
bar_data_muscle_ob = [sum(THalf_liver(index_OB,:)<threshold_THalf_1),sum((THalf_liver(index_OB,:)>=threshold_THalf_1).*(THalf_liver(index_OB,:)<=threshold_THalf_2)),sum(THalf_liver(index_OB,:)>threshold_THalf_2)];
bar_X = [1,2];
bar_Y = [bar_data_muscle_WT;bar_data_muscle_ob];
bar_XLabel = ["WT","\it{ob}\rm{/}\it{ob}"];
bar_XLabel_TissueType = "Muscle";
