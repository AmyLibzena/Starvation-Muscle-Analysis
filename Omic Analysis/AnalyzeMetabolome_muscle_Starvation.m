%% Load files
load('./Resource/MetabolomeData_muscle.mat');
index_F16P = find(strcmp(Info_MetabolomeName(:,2),'F1,6P'));
index_F26P = size(Info_MetabolomeName,1) + 1;
Metabolome_muscle(:,index_F16P) = Extra_ICQEMS(:,1);
Metabolome_muscle(:,index_F26P) = Extra_ICQEMS(:,2);
...
%% Save
exportgraphics(f1,['./Fig/','EnergyCharge','.pdf'],'ContentType','vector');
exportgraphics(f2,['./Fig/','AMP2ATP','.pdf'],'ContentType','vector');
exportgraphics(f3,['./Fig/','ADP2ATP','.pdf'],'ContentType','vector');
...
bar_XLabel_OmicType = "Metabolome";
f = figure(1);
hold on;
...
for i = 1:num_TissueType
    ...
    text(temp_X_pos,-7,bar_XLabel_OmicType,'HorizontalAlignment','center','VerticalAlignment','middle','FontSize',15)
end
...
%temp_num_Cluster = 2;
%name_figure_1 = 'Metabolome_MuscleWT';
%name_figure_2 = 'Metabolome_MeanTimeCourseInOne_MuscleWT';

temp_num_Cluster = 3;
name_figure_1 = 'Metabolome_MuscleOB';
name_figure_2 = 'Metabolome_MeanTimeCourseInOne_MuscleOB';
...
%temp_data = log2normalize_muscleWT(:,temp_include);
temp_data = log2normalize_muscleOB(:,temp_include);
...
%%% Heat map 1
subplot(1,4,2);
temp_XLabel = {'0','2','4','6','8','12','16','24'};
temp_YLabel = Info_MetabolomeName(:,2);
for i = 1:length(temp_YLabel)
    temp_list = is_Responsive(1:num_Genotype,i);
    if (temp_list(1)~=0)&&(temp_list(1)==temp_list(2))
...
%temp_data = is_Responsive(index_WT,temp_include);
temp_data = is_Responsive(index_OB,temp_include);
...
%temp_data = THalfType_muscle(index_WT,temp_include);
temp_data = THalfType_muscle(index_OB,temp_include);
...
for i = 1:temp_Cluster_list(end)
    ...
    hold on;
    %errorbar(temp_x,temp_data_cluster_mean(1:num_TimePoint),temp_data_cluster_error(1:num_TimePoint),'Color',color_WT,'LineWidth',1);
    errorbar(temp_x,temp_data_cluster_mean(1:num_TimePoint),temp_data_cluster_error(1:num_TimePoint),'Color',color_OB,'LineWidth',1);
end
%legend(["WT","\it{ob}\rm{/}\it{ob}"]);
f2.Position = [0 0 200 700];
