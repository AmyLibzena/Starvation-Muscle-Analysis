%% Load files
load('./Resource/WB_OGTTLiver.mat');
...
is_TimeCourseDifferent_percentage = false(1,size(is_Different_percentage,2));
is_TimeCourseDifferent_percentage = is_TimeCourseDifferent_percentage + (sum(logical(is_Different_percentage))~=0).*(logical(~sum(is_Different_percentage==-1))).*(1);
is_TimeCourseDifferent_percentage = is_TimeCourseDifferent_percentage + (sum(logical(is_Different_percentage))~=0).*(logical(~sum(is_Different_percentage==1))).*(-1);
is_TimeCourseDifferent_percentage = is_TimeCourseDifferent_percentage + (sum(logical(is_Different_percentage))~=0).*(logical(sum(is_Different_percentage==1)).*logical(sum(is_Different_percentage==-1))).*(2);
