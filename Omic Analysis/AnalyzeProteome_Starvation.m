%% Load files
load('./Resource/ProteomeData.mat');
...
for i = 1:size(is_EnoughData,2)
    logic_EnoughData = CheckEnoughData(Proteome_muscle(:,i), col_num_muscle);
    is_EnoughData(index_muscle,i) = logic_EnoughData;
end

%%% Calculation
%%% Mean
mean_muscle = NaN(length(col_num_muscle),size(Info_ProteomeName,1));
for i = 1:size(mean_muscle,2)
    mean_muscle(:,i) = GetMean(Proteome_muscle(:,i),col_num_muscle);
end
...
p_muscle = NaN(num_Genotype,size(Info_ProteomeName,1));
temp_num = sum(col_num_muscle(1:num_TimePoint));
% Muscle - WT
for i = 1:size(Info_ProteomeName,1)
    p_muscle(index_WT,i) = anova1(Proteome_muscle(1:temp_num,i),Group(1:temp_num),'off');
end
% Muscle - OB
for i = 1:size(Info_ProteomeName,1)
    p_muscle(index_OB,i) = anova1(Proteome_muscle(temp_num+1:end,i),Group(temp_num+1:end),'off');
end
% Muscle - OB/WT
for i = 1:size(Info_ProteomeName,1)
    for j = 1:num_TimePoint
        if j == 1
            temp_list_WT = Proteome_muscle(1:col_num_muscle(j),i);
            temp_list_OB = Proteome_muscle(temp_num+1:temp_num+col_num_muscle(num_TimePoint+j),i);
        else
            temp_list_WT = Proteome_muscle(sum(col_num_muscle(1:j-1))+1:sum(col_num_muscle(1:j)),i);
            temp_list_OB = Proteome_muscle(sum(col_num_muscle(1:num_TimePoint+j-1))+1:sum(col_num_muscle(1:num_TimePoint+j)),i);
        end
...
% {"<8 h","8-16 h",">16 h"}
THalf_muscle = NaN(num_Genotype,size(Info_ProteomeName,1));
